...
`),
  ...
  it('rejects pure setup messages', () => {
    expect(
      extractMeaningfulPrompt('<system_instruction>internal setup</system_instruction>'),
    ).toBeUndefined();
    expect(
      extractMeaningfulPrompt('<environment_context>{"cwd":"/repo"}</environment_context>'),
    ).toBeUndefined();
    expect(
      extractMeaningfulPrompt('Respond directly to the user without extra narration.'),
    ).toBeUndefined();
  });
