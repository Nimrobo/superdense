...
function workspaceStorageDir(): string {
  return (
    process.env.CURSOR_WORKSPACE_STORAGE_DIR ??
    DEFAULT_CURSOR_WORKSPACE_STORAGE_DIR
  );
}
...
function firstPromptFromHeaders(
  ...
  for (const h of headers) {
    ...
    const row = stmt.get(`bubbleId:${composerId}:${h.bubbleId}`) as
      | { value?: string }
      | undefined;
...
function maxBubbleMtime(
  ...
  const row = stmt.get(`bubbleId:${composerId}:${tail.bubbleId}`) as
    | { value?: string }
    | undefined;
  const bubble = safeJsonParse<CursorBubble>(row?.value);
  ...
  async discover(): Promise<DiscoveredSession[]> {
    ...
    try {
      ...
      for (const row of rows) {
        ...
        const composerId =
          composer?.composerId ?? row.key.slice("composerData:".length);
...
async function* iterCursorEvents(
  logPath: string,
  ...
  try {
    ...
    for (const header of headers) {
      ...
      const isUser =
        (typeof bubble.type === "number" ? bubble.type : header.type) === 1;
...
function* bubbleToEvents(
  ...
  if (type === 2) {
    ...
    if (tfd && typeof tfd === "object") {
      const toolCallId =
        typeof tfd.toolCallId === "string" ? tfd.toolCallId : undefined;
