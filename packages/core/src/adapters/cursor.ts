...
function fileUriToPath(folder: unknown): string | undefined {
  ...
  try { return fileURLToPath(folder); } catch { return undefined; }
}
...
function deriveMode(bubble: CursorBubble, isUser: boolean, lastMode: string | undefined): string | undefined {
...
async function buildComposerPwdMap(): Promise<Map<string, string>> {
  ...
  for (const entry of entries) {
    ...
    try {
      ...
      const parsed = safeJsonParse<{ allComposers?: Array<{ composerId?: string }> }>(row?.value);
...
function parseComposerIdFromLogPath(logPath: string): { dbPath: string; composerId?: string } {
  ...
  async discover(): Promise<DiscoveredSession[]> {
    ...
    try {
      ...
      for (const row of rows) {
        ...
        const firstPrompt = firstPromptFromHeaders(db, composerId, headers, composer?.name ?? composer?.text ?? undefined);
...
async function* iterCursorEvents(logPath: string): AsyncIterable<TranscriptEvent> {
  ...
  try {
    ...
    for (const header of headers) {
      ...
      const row = bubbleStmt.get(`bubbleId:${composerId}:${header.bubbleId}`) as { value?: string } | undefined;
      const bubble = safeJsonParse<CursorBubble>(row?.value);
      if (!bubble) continue;
...
function* bubbleToEvents(bubble: CursorBubble, headerType: number | undefined): Generator<TranscriptEvent> {
  const ts = parseCreatedAtMs(bubble.createdAt);
  ...
  if (type === 2) {
    if (typeof bubble.text === 'string' && bubble.text.trim()) {
      yield { ts, kind: 'text', role: 'assistant', text: bubble.text, raw: bubble };
    }
    ...
    if (tfd && typeof tfd === 'object') {
      ...
      const inputText = typeof tfd.rawArgs === 'string' && tfd.rawArgs
        ? tfd.rawArgs
        : (typeof tfd.params === 'string' ? tfd.params : stringifyValue(tfd.params ?? {}));
      yield {
        ts,
        kind: 'tool_call',
        role: 'assistant',
