...
out="$(superdense skill install superdense 2>&1)"; rc=$?
...
out2="$(superdense skill install superdense 2>&1)"; rc=$?
expect_eq "skill install repeat exit code" "0" "$rc"

# 17. --locally writes under cwd
work_cwd="$(mktemp -d)"
( cd "$work_cwd" && superdense skill install superdense --locally >/dev/null )
