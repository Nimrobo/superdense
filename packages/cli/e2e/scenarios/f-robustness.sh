...
road42 discover >/dev/null 2>&1
road42 index >/dev/null 2>&1
out="$(road42 session list 2>&1)"; rc=$?
...
echo "this is not a sqlite file" > "$ROAD42_HOME/index.db"
err="$(road42 session list 2>&1 || true)"
...
new_home >/dev/null
err2="$(road42 query --query 'not-json' 2>&1)"; rc=$?
...
err3="$(road42 totally-not-a-command 2>&1)"; rc=$?
