...
7. Define `gate.md` as reusable completion policy, not a per-run checklist. Include the standard sections from `references/outcome-loop.md`: `# Gate`, `## Completion Rules`, `## Required Checks`, `## Warning Checks`, `## Deterministic Checks`, and `## Failure Policy`. It may be operationally empty when no checks are known yet; mark lists as `none configured` instead of inventing fake checks.
8. Add local deterministic check scripts only when this outcome needs them. Keep them inside the outcome folder, reference them from `gate.md`, and do not add bundled or generic script templates.
9. If analytics/instrumentation is missing and the target repo is available, implement the smallest viable instrumentation in that target repo after inspecting local conventions. Never commit private credentials. Treat public client tokens according to target repo conventions.
10. If external account setup is required, pause with a concrete checklist for the human. Continue once the human provides access or confirms setup.
...
- `gate.md` is reusable completion policy. It defines required checks, warning checks, deterministic checks when needed, and the failure policy for every run.
