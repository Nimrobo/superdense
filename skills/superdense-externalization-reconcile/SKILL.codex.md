...
3. Decide whether the artifact stayed internal or infer each connector it needs.
4. Run `superdense externalization connector list`. Connector CLIs are user-installed and
   user-configured. Their repositories define their read-only search commands and opaque locator
   formats. Do not install or configure connectors. Do not attach matches from general web search.
5. Persist a complete replacement assessment with
   `superdense externalization assess --input '<json>'`.
6. Confirm with `superdense externalization show <artifact-id>`.
7. Stop after the current page. Report `nextCursor`; when it is non-null, print the exact
