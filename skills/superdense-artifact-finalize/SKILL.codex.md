...
3. If the thread is still `open`, finalize it first:
   `superdense curation apply --input '{"actions":[{"type":"thread.finalize","threadId":"<thread-id>"}]}'`.
4. Extract the artifact with `superdense artifact finalize --input '<json>'`, choosing an
...
5. Confirm with `superdense artifact show <thread-id>` and report.
