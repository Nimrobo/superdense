...
2. For each linked target, gather its current real-world metrics using whatever tool you have: a platform CLI, the provider's own API, or the open web. Superdense neither installs nor runs connectors; you collect the numbers. The target's `connector` is just a free-text platform label.
...
- Reuse a loose shared vocabulary across connectors where it applies, for example `reach`, `engagement`, `reactions`, `conversions`, so later cohort comparison can align on common axes while still keeping connector-specific dimensions.
